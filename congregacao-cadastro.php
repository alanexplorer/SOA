<?php
    ...
    {
        $ucongre = strip_tags($_POST['txt_congre']);
        $ulider = strip_tags($_POST['txt_lider']);
        $uadress = strip_tags($_POST['txt_adress']);	
        
        if($ucongre=="")	{
            $error[] = "digite o nome da congregação !";	
        }
        else if($ulider=="")	{
        ...
        else
        {
            try
            {
                $stmt = $user->runQuery("SELECT con_name, con_adress FROM congregation WHERE con_name=:ucongre OR con_adress=:uadress");
                $stmt->execute(array(':ucongre'=>$ucongre, ':uadress'=>$uadress));
                $row=$stmt->fetch(PDO::FETCH_ASSOC);
                    
                if($row['con_name']==$ucongre) {
                    $error[] = "O nome da congregação já existe!";
                }
                else if($row['con_adress']==$uadress) {
                    $error[] = "Ja éxiste uma congregação neste endereço!";
                }
                else
                {
                    if($user->register($ucongre,$ulider,$uadress)){	
                        $user->redirect('congregacao-cadastro.php?joined');
                    }
                }
            }
          ...
          <!-- form line 1 -->
                    <div class="form-row">
                        <div class="col-sm-3 mb-3">
                            ...
                            <input type="text" class="form-control" id="txt_congre" value="<?php if(isset($error)){echo $ucongre;}?>" required name="txt_congre">
                    ...
                    <div class="col-sm-4 mb-3">
                      		...
                      		<input type="text" class="form-control" id="txt_lider" value="<?php if(isset($error)){echo $ulider;}?>" required name="txt_lider">
                      		<div class="invalid-feedback">
                     ...
                     <div class="col-sm-5 mb-3">
                      		...
                      		<input type="text" class="form-control" id="txt_adress" value="" required name="txt_adress">
                      <div class="invalid-feedback">
                        Digite o endereço!
                      	</div>
                    </div>
            ...
            <!-- fim form line 1 -->
                  <button class="btn btn-primary" type="submit" name="btn-signup"><i class="fa fa-save"></i> Cadastrar</button>
                  <button class="btn btn-danger" type="reset" name="reset"><i class="fa fa-broom"></i> Recomeçar</button>
                   <?php
                    if(isset($error))
                    {
                    ...
                    else if(isset($_GET['joined']))
                    {
                         ...
                         <div class="alert alert-info">
                              <i class="fa fa-thumbs-up"></i> &nbsp; Congregação cadastrada !
                         </div>
                         <?php
                    }
                    ?>
